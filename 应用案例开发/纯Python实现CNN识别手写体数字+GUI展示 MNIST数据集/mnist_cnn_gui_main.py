...
class MainWindow(QMainWindow, Ui_MainWindow):
    def __init__(self):
        ...
        self.paintBoard = PaintBoard(
            self, Size=QSize(224, 224), Fill=QColor(0, 0, 0, 0))
    ...
    def pbtPredict_Callback(self):
        ...
        if self.mode == MODE_MNIST:
            ...
            if __img == None:   # 无图像则用纯黑代替
                ...
                __img = ImageQt.ImageQt(Image.fromarray(
                    np.uint8(np.zeros([224, 224]))))
            else:
                __img = __img.toImage()
        ...
        img_array = np.array(pil_img.convert(
            'L')).reshape(1, 1, 28, 28) / 255.0
